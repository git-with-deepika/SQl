...
-- Dump completed on 2020-01-22  9:56:18
 ...
 select count ( Name ) ,count(  distinct Continent ), avg (Population ) , sum (Capital) from country where name like 'A%' or nme like 'd%';
 
 -- group by
 select Continent, count(*) from  country group by Continent;
 select GovernmentForm , count(*) from country group by GovernmentFrom;
 select  continent , count(name) , sum(Population),avg(Population),max(Population) ,max(Indepyear),min(Population), min(Indepyear) from country group by continent;
